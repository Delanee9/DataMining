...
class DataHandler {
    ...
    /**
     * Generate starting centroids by selection random entry from the dataset.
     *
     * @param data        ArrayList<SingleInstance>
     * @param numClusters int
     * @return ArrayList<Centroid>
     */
    private static ArrayList<Centroid> calculateStartingCentroids(ArrayList<SingleInstance> data, int numClusters) {
        ArrayList<Centroid> centroidList = new ArrayList<>();
        while(centroidList.size() < numClusters) {
            SingleInstance dataRow = data.get(randInt(data.size() - 1));
    ...
    /**
     * Method to output the result set to the console.
     *
     * @param dataset ArrayList<SingleInstance>
     */
    private static void printTableToConsole(ArrayList<SingleInstance> dataset) {
        ...
        for(SingleInstance sI : dataset) {
            System.out.format("%10s%12f%12f%12f%5d%10d%2s%5d%2s%n", sI.getDateEntry().getDate(), sI.getPollution().getNitricOxide(), sI.getPollution().getOzone(), sI.getPollution().getSulphurDioxide(), sI.getBikeData().getHireTime(), sI.getBikeData().getBikeHires(), "|", sI.getClusterClass(), "|");
        }
    }
}